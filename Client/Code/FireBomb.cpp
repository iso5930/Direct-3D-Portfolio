...
CFireBomb::CFireBomb(LPDIRECT3DDEVICE9 pDevice)
...
CFireBomb* CFireBomb::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CFireBomb* pEffect = new CFireBomb(pDevice);
...
HRESULT CFireBomb::Initialize(void)
{
...
HRESULT CFireBomb::AddComponent(void)
{
	...
	//Texture
	pComponent = Engine::Get_ResourceMgr()->CloneResources(Resource_Static, L"FireBall_Bomb");
...
void CFireBomb::SetInfo(D3DXVECTOR3 vPos)
{
	m_pInfo->m_vPos = vPos;
}