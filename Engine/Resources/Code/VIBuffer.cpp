...
:CResources(pDevice), m_pVB(NULL), m_dwVtxSize(0), m_dwVtxCnt(0), m_dwVtxFVF(0),
m_pIB(NULL), m_dwIdxSize(0), m_dwTriCnt(0), m_IdxFmt(D3DFMT_UNKNOWN)
{

}

Engine::CVIBuffer::CVIBuffer(const CVIBuffer& rhs)
: CResources(rhs.m_pDevice), m_pVB(rhs.m_pVB), m_dwVtxSize(rhs.m_dwVtxSize), m_dwVtxCnt(rhs.m_dwVtxCnt)
, m_dwVtxFVF(rhs.m_dwVtxFVF), m_pIB(rhs.m_pIB), m_dwIdxSize(rhs.m_dwIdxSize)
, m_dwTriCnt(rhs.m_dwTriCnt), m_IdxFmt(rhs.m_IdxFmt)
{
	++m_dwRefCnt;
}
...
void Engine::CVIBuffer::SetIdxInfo(void* pIdxInfo, DWORD* pTriCnt)
{
	void* pOriIndex = NULL;
